...
def display_users_data(privilege_level):
    if privilege_level == 1:  # Only allow admin to view users' data
        ...
        if len(data) > 0:
            st.header("Data in 'users' Table")
            for row in data:
                st.write("Username:", row[0])
                st.write("Privilege Level:", row[2])
                st.write("---")
        else:
            st.info("No data available in the users' table.")
...
def authenticate(username, password):
    ...
    if result:
        return True, result[2]  # Return True and the user's privilege level
    else:
        return False, None
...
def login():
    ...
    if not logged_in:
        ...
        if st.button("Login"):
            authenticated, privilege_level = authenticate(username, password)

            if authenticated:
                st.session_state.logged_in = True
                st.session_state.privilege_level = privilege_level
                st.success("Logged in as admin" if privilege_level == 1 else "Logged in as user")
                return True
            else:
...
def register_user(cursor):
    if st.session_state.privilege_level == 1:  # Only allow admin to register a new user
        ...
        if st.button("Register"):
            ...
            if result:
                ...
                # Insert the new user into the 'users' table with privilege_level=0 (regular user)
                query = "INSERT INTO users (USERNAME, PASSWORD, PRIVILEGE) VALUES (%s, %s, 0)"
...
def delete_user():
    if st.session_state.privilege_level == 1:  # Only allow admin to delete a user
        st.subheader("Delete User")

        user_to_delete = st.text_input("Enter the Username of the user to delete")

        # Use user_to_delete as the key for the "Delete" button
        if st.button("Delete", key=f"delete_{user_to_delete}"):
            ...
            if result:
                ...
                st.error(f"User '{user_to_delete}' not found in the database.")
    
def logout():
    st.session_state.logged_in = False
    st.session_state.privilege_level = None
    st.info("Logged out successfully!")

def display_data2(privilege_level):
...
def add(privilege_level):
    # Check if the user has admin privilege (privilege_level == 1)
    if privilege_level == 1:
        ...
        return "You don't have the privilege to add data."
...
def perform_ocr(image_path):
    ...
    if resp_dict and 'results' in resp_dict[0] and resp_dict[0]['results']:
...
def open_camera():
    ...
    plateCascade = cv2.CascadeClassifier(r"C:\Users\imelv\OneDrive\Desktop\ANPR-System\haarcascade_russian_plate_number")
    minArea = 500
    ...
    while camera_open:
        ...
        if cv2.waitKey(1) & 0xFF == ord('s'):
            cv2.imwrite(r"C:\Users\imelv\OneDrive\Desktop\ANPR-System\scan" + str(count) + ".jpg", imgRoi)
...
def main():
    ...
    **Supervisor:**
    ...
    display_data2(privilege_level=st.session_state.privilege_level)

    if st.session_state.privilege_level == 1:  # Admin privilege
        ...
        if st.session_state.privilege_level == 1:  # Admin privilege
         st.title("View and Manage Registered Users")

        # Display registered users' data
        display_users_data(privilege_level=st.session_state.privilege_level)
